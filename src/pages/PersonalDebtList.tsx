...
import { personalDebtService } from '../services/api';
import Table from '../components/Table.tsx';
import Modal from '../components/Modal.tsx';
import PersonalDebtForm from '../components/PersonalDebtForm.tsx';
...
import { PersonalDebt, TableColumn } from '../types';
...
export default function PersonalDebtList() {
  const isMobile = useIsMobile();
  const location = useLocation();

  const [items, setItems] = useState<PersonalDebt[]>([]);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [selectedItem, setSelectedItem] = useState<PersonalDebt | null>(null);
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState<boolean>(false);
  const [itemToDelete, setItemToDelete] = useState<PersonalDebt | null>(null);
  ...
  const loadItems = async (): Promise<void> => {
    try {
      ...
      const response = await personalDebtService.getAll({ startDate, endDate }, currentPage, 20);
      ...
      console.error('Error loading personal debts:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (item: PersonalDebt): void => { setSelectedItem(item); setIsModalOpen(true); };
  const handleDelete = (item: PersonalDebt): void => { setItemToDelete(item); setIsDeleteModalOpen(true); };

  const confirmDelete = async (): Promise<void> => {
    if (!itemToDelete?.id) return;
    try {
      await personalDebtService.delete(itemToDelete.id);
      loadItems();
      setIsDeleteModalOpen(false);
      setItemToDelete(null);
    } catch (err) {
      console.error('Error deleting personal debt:', err);
      alert('Error al eliminar la deuda');
    }
  };

  const handleSubmit = async (formData: PersonalDebt): Promise<void> => {
    try {
      if (selectedItem?.id) {
        const cancelledChanged = formData.cancelled !== selectedItem.cancelled;
        await personalDebtService.update(selectedItem.id, formData);
        if (cancelledChanged) {
          if (formData.cancelled === true) {
            await personalDebtService.cancel(selectedItem.id);
          } else {
            await personalDebtService.uncancel(selectedItem.id);
          }
        }
      } else {
        await personalDebtService.create(formData);
      }
      loadItems();
      setIsModalOpen(false);
      setSelectedItem(null);
    } catch (err) {
      console.error('Error saving personal debt:', err);
  ...
  const chartSlices: DonutSlice[] = (() => {
    ...
    filtered.forEach(item => {
      const key = item.creditor || 'Sin acreedor';
  ...
  const modals = (
    <>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={selectedItem ? 'Editar Deuda' : 'Registrar Deuda'}>
        <PersonalDebtForm item={selectedItem} onSubmit={handleSubmit} onCancel={() => setIsModalOpen(false)} />
      </Modal>
      ...
      <div className="animate-fade-in">
        <div className="flex items-center justify-between px-4 pt-5 pb-2">
          <h1 className="text-2xl font-bold text-stone-900 dark:text-stone-50 flex items-center gap-2">
            <ClipboardList size={22} className="text-teal-700 dark:text-teal-400" />Deudas
          </h1>
          <button
            ...
            items.map((item) => (
              ...
              >
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${
                  ...
                  <svg className={`w-5 h-5 ${item.cancelled ? 'text-green-600 dark:text-green-400' : 'text-purple-600 dark:text-purple-400'}`}
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {item.amountInDollars != null ? (
                        <>
                          ...
                          <p className="font-semibold text-purple-600 dark:text-purple-400 text-sm">{formatUSD(item.amountInDollars)}</p>
                        </>
                      ) : (
                        <>
                          ...
                          <p className="font-semibold text-purple-600 dark:text-purple-400 text-sm">{formatCurrency(item.amountInPesos ?? 0)}</p>
                        </>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-1">
                    <div className="flex items-center gap-2 min-w-0 flex-wrap">
                      ...
                      <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${
                        ...
                        {item.cancelled ? 'Cancelada' : 'Pendiente'}
                      </span>
                    </div>
  ...
  // ── DESKTOP VIEW ───────────────────────────────────────────────────────────
  const columns: TableColumn<PersonalDebt>[] = [
    ...
    },
    {
      ...
      render: (value: string) => <span className="text-stone-700 dark:text-stone-300">{value || '-'}</span>,
    },
    {
      key: 'amountInPesos', label: 'Monto',
      render: (_: number, row: PersonalDebt) => (
        <div className="flex items-center gap-1.5">
          {row.amountInDollars != null ? (
            <>
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400">USD</span>
              <span className="font-semibold text-purple-700 dark:text-purple-400">{formatUSD(row.amountInDollars)}</span>
            </>
          ) : (
            <>
              <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-stone-100 text-stone-500 dark:bg-stone-800 dark:text-stone-400">ARS</span>
              <span className="font-semibold text-purple-700 dark:text-purple-400">{formatCurrency(row.amountInPesos ?? 0)}</span>
            </>
          )}
        </div>
      ),
        ...
        <div className="mb-8 animate-fade-in flex items-start justify-between">
          <div>
            <h1 className="text-4xl font-bold text-stone-900 dark:text-stone-50 mb-2 flex items-center gap-3">
              <ClipboardList size={36} className="text-teal-700 dark:text-teal-400" />Deudas Personales
            </h1>
            <p className="text-stone-600 dark:text-stone-400">Administra tus deudas con personas</p>
          </div>
          <MonthPicker year={selectedYear} month={selectedMonth} onChange={handleMonthChange} />
        </div>
